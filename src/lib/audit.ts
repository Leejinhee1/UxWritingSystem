...
export interface PreparedRules {
  ...
  wordListRules: RuleData[];
  grammarRules: RuleData[];
  ruleNameMap: Record<string, string>;
}

// === Rule Preparation (1회 로딩, 여러 문구에 재사용) ===

export function prepareRules(rules: RuleData[]): PreparedRules {
  ...
  return { voice, principles, grammars, patterns, wordListRules, grammarRules, ruleNameMap };
}
...
export async function auditText(
...
): Promise<Omit<AuditResult, "audit_id">> {
  // === Rule-based checks (L5) ===
  const ruleViolations: Violation[] = [];

  for (const rule of prepared.wordListRules) {
  ...
  const systemPrompt = buildSystemPrompt(
    prepared.voice, prepared.principles, prepared.grammars, prepared.patterns,
    ruleViolations, uiType, prepared.ruleNameMap,
  );
  ...
  for (const v of ruleViolations) {
    if (v.ruleType === "grammar") {
      const rule = prepared.grammarRules.find((r) => r.id === v.ruleId);
  ...
  const suggestions = [...ruleSuggestions, ...(aiResult?.suggestions || [])];
  const pass = violations.length === 0;

  const combined = aiResult?.combined_suggestion || null;
  const combinedResult = pass ? null : (combined || text).trim();
  const combined_suggestion = combinedResult && combinedResult !== text.trim() ? combinedResult : null;
...
${principles.map((p) => {
  ...
  if (pat.weeds) { rendered.add("weeds"); detail += `\n  잡초 단어 목록: ${(pat.weeds as string[]).join(", ")}`; }
  if (pat.examples) {
    rendered.add("examples");
    for (const ex of (pat.examples as Array<{ do: string; dont: string; context?: string }>)) {
      detail += `\n  Do: "${ex.do}" / Don't: "${ex.dont}"`;
      if (ex.context) detail += ` (${ex.context})`;
    }
  }
...
${grammars.map((g) => {
  ...
  if (pat.exception) {
    rendered.add("exception");
    detail += `\n  예외 (이 유형에서는 검사 안 함): ${(pat.exception as string[]).join(", ")}`;
  }
  for (const [k, v] of Object.entries(pat)) {
    if (!rendered.has(k)) detail += `\n  ${k}: ${typeof v === "string" ? v : JSON.stringify(v)}`;
  }
...
${patterns.map((p) => {
  const pat = p.pattern;
  const rendered = new Set<string>(["ui_type"]);
  let detail = `- ${pat.ui_type} (${p.name})`;
  if (pat.tone) { rendered.add("tone"); detail += `\n  톤: ${pat.tone}`; }
  if (pat.pattern_rule) { rendered.add("pattern_rule"); detail += `\n  패턴: ${pat.pattern_rule}`; }
  if ((pat.skip_rules as string[])?.length) {
    rendered.add("skip_rules");
    const skipNames = (pat.skip_rules as string[]).map(id => id === "ALL" ? "ALL" : ruleNameMap[id] || id);
    detail += `\n  이 유형에서 건너뛸 규칙: ${skipNames.join(", ")}`;
  } else {
    rendered.add("skip_rules");
  }
  if (pat.note) { rendered.add("note"); detail += `\n  참고: ${pat.note}`; }
  if ((pat.examples as Array<{ do: string; dont: string; context?: string }>)?.length) {
    rendered.add("examples");
    for (const ex of (pat.examples as Array<{ do: string; dont: string; context?: string }>)) {
      detail += `\n  Do: "${ex.do}" / Don't: "${ex.dont}"`;
      if (ex.context) detail += ` (${ex.context})`;
